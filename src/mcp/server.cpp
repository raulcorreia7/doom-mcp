...
static bool HandleMCPRequest(Server* server, const char* body,
  ...
  *http_status = 200;
  ...
  if (!doc.parse(body)) {
    ...
    Builder b;
    b.start_object();
    b.add("jsonrpc", MCP_JSONRPC_VERSION);
    Builder result;
...
static bool OnHttpRequest(void* user_data, const char* method, const char* path,
  ...
  *http_status       = 404;
  ...
  if (std::strcmp(method, "POST") == 0 &&
      std::strcmp(path, MCP_ENDPOINT_MCP) == 0) {
    return HandleMCPRequest(server, body ? body : "", response_buffer,
                            response_size, http_status);
  }

  if (std::strcmp(method, "GET") == 0 &&
      std::strcmp(path, MCP_ENDPOINT_HEALTH) == 0) {
    Builder b;
    b.start_object();
    b.add("status", "ok");
...
mcp_server_t* mcp_server_create(const mcp_server_config_t* config) {
  ...
  server->transport =
      mcp_sse_transport.create(server->config.port, &callbacks, server);
...
size_t mcp_format_error_response(char* buffer, size_t buffer_size,
  ...
  std::string response = mcp::json::make_jsonrpc_error(
      id ? id : "null", error_code,
