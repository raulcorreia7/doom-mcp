...
static std::string generate_session_id(Server* server) {
...
static std::shared_ptr<Session> find_session_by_id(Server* server, std::string_view session_id) {
...
static std::shared_ptr<Session> create_session(Server* server, std::string& out_session_id) {
  if (!server) {
    return nullptr;
  }

  for (int attempt = 0; attempt < 8; ++attempt) {
    const std::string candidate = generate_session_id(server);
...
static bool delete_session_by_id(Server* server, std::string_view session_id) {
  if (!server || session_id.empty()) {
    return false;
  }
...
static std::string extract_session_id(const Value& params) {
...
static void server_log(const Server* server, int level, const char* fmt, ...) {
...
static std::string normalize_http_path(std::string_view path) {
  if (path.empty()) {
    return "/";
  }
  std::string out(path);
...
static std::string build_route_key(std::string_view method, std::string_view path) {
  std::string key = normalize_http_method(method);
  key.push_back(' ');
  key += normalize_http_path(path);
  return key;
}

static bool is_reserved_route_key(std::string_view route_key) {
  return route_key == build_route_key("POST", MCP_ENDPOINT_MCP) ||
         route_key == build_route_key("DELETE", MCP_ENDPOINT_MCP) ||
         route_key == build_route_key("GET", MCP_ENDPOINT_HEALTH);
}
...
static bool is_supported_protocol_version(std::string_view requested_version) {
...
static bool requires_initialized_notification(std::string_view protocol_version) {
  return protocol_version == MCP_PROTOCOL_VERSION;
}

static std::string build_unsupported_protocol_data(std::string_view requested_version) {
  Builder data;
  data.start_object();
...
static bool handle_mcp_request(Server* server, const char* body, char* response_buffer,
                               size_t response_size, int* http_status) {
  *http_status = 200;
  server_log(server, MCP_LOG_DEBUG, "HTTP POST %s", MCP_ENDPOINT_MCP);
  request_context::set_response_protocol_version(MCP_PROTOCOL_VERSION);

  Document doc;
  if (!doc.parse(body ? body : "")) {
    server_log(server, MCP_LOG_WARN, "Parse error on %s", MCP_ENDPOINT_MCP);
    std::string resp = build_json_rpc_error("null", -32700, "Parse error");
  ...
  Value req = doc.root();
  if (!req.is_object()) {
    server_log(server, MCP_LOG_WARN, "Invalid JSON-RPC request: root is not object");
    std::string resp = build_json_rpc_error("null", -32600, "Invalid Request");
    if (resp.size() >= response_size) {
      *http_status = 500;
      return false;
    }
  ...
  if (!is_notification) {
    ...
    if (!is_valid_json_rpc_id(id_value)) {
      server_log(server, MCP_LOG_WARN, "Invalid JSON-RPC id");
      std::string resp = build_json_rpc_error("null", -32600, "Invalid Request");
      if (resp.size() >= response_size) {
        *http_status = 500;
        return false;
  ...
  if (!jsonrpc_value.is_string() ||
    ...
    server_log(server, MCP_LOG_WARN, "Invalid JSON-RPC request: jsonrpc must be %s",
               MCP_JSONRPC_VERSION);
    std::string resp = build_json_rpc_error(id_json, -32600, "Invalid Request");
    if (resp.size() >= response_size) {
      *http_status = 500;
      return false;
    }
  ...
  if (!method_value.is_string()) {
    server_log(server, MCP_LOG_WARN, "Invalid JSON-RPC request: method missing or not string");
    std::string resp = build_json_rpc_error(id_json, -32600, "Invalid Request");
    if (resp.size() >= response_size) {
      *http_status = 500;
      return false;
  ...
  server_log(server, MCP_LOG_DEBUG, "JSON-RPC method=%s notification=%s", method.c_str(),
             is_notification ? "true" : "false");

  if (method.empty()) {
    server_log(server, MCP_LOG_WARN, "Invalid JSON-RPC request: empty method");
    std::string resp = build_json_rpc_error(id_json, -32600, "Invalid Request");
    if (resp.size() >= response_size) {
      *http_status = 500;
      return false;
  ...
  if (method == "initialize") {
    server_log(server, MCP_LOG_DEBUG, "MCP initialize request received");

    if (is_notification) {
      std::string resp = build_json_rpc_error("null", -32600, "initialize must be a request");
      if (resp.size() >= response_size) {
        *http_status = 500;
        return false;
      }
    ...
    if (!params.is_object()) {
      std::string resp =
          build_json_rpc_error(id_json, -32602, "initialize params must be an object");
      if (resp.size() >= response_size) {
        *http_status = 500;
        return false;
      }
    ...
    if (requested_version.empty()) {
      std::string resp =
          build_json_rpc_error(id_json, -32602, "initialize.params.protocolVersion is required");
      if (resp.size() >= response_size) {
        *http_status = 500;
        return false;
      }
    ...
    if (!is_supported_protocol_version(requested_version)) {
      std::string resp = build_json_rpc_error(id_json, -32602, "Unsupported protocol version",
                                              build_unsupported_protocol_data(requested_version));
      if (resp.size() >= response_size) {
        *http_status = 500;
        return false;
      }
    ...
    if (!params["capabilities"].is_object()) {
      std::string resp =
          build_json_rpc_error(id_json, -32602, "initialize.params.capabilities is required");
      if (resp.size() >= response_size) {
        *http_status = 500;
        return false;
      }
    ...
    if (!params["clientInfo"].is_object()) {
      std::string resp =
          build_json_rpc_error(id_json, -32602, "initialize.params.clientInfo is required");
      if (resp.size() >= response_size) {
        *http_status = 500;
        return false;
      }
    ...
    std::shared_ptr<Session> session = create_session(server, session_id);
    if (!session) {
      const std::string resp = build_json_rpc_error(id_json, -32603, "Failed to create session");
      if (resp.size() >= response_size) {
        *http_status = 500;
        return false;
      }
    ...
    session->requires_initialized_notification.store(
        requires_initialized_notification(session->protocol_version), std::memory_order_release);
    ...
    std::string resp = build_json_rpc_result(id_json, b.finish());
  ...
  if (method == "notifications/initialized") {
    server_log(server, MCP_LOG_DEBUG, "MCP notifications/initialized received");

    std::string                    session_id = extract_session_id(req["params"]);
    const std::shared_ptr<Session> session    = find_session_by_id(server, session_id);
    ...
    if (!session || !session->initialize_completed.load(std::memory_order_acquire)) {
      ...
      const std::string resp = build_json_rpc_error(
          id_json, kJsonRpcServerNotInitialized, "Server not initialized: call initialize first");
      if (resp.size() >= response_size) {
        *http_status = 500;
        return false;
      }
    ...
    if (is_notification) {
      *http_status       = 202;
      ...
      const std::string resp = build_json_rpc_result(id_json, "{}");
      if (resp.size() >= response_size) {
        *http_status = 500;
        return false;
      }
  ...
  if (!session || !session->initialize_completed.load(std::memory_order_acquire) ||
    ...
    server_log(server, MCP_LOG_WARN, "Request before initialization complete: method=%s session=%s",
  ...
  if (method == "ping") {
    server_log(server, MCP_LOG_DEBUG, "MCP ping request received");
    if (is_notification) {
      *http_status       = 202;
      response_buffer[0] = '\0';
    } else {
  ...
  if (found_handler) {
    server_log(server, MCP_LOG_DEBUG, "Method found in registry: %s", method.c_str());
    Value params = req["params"];
    ...
    if (handler.fn(handler.user_data, method.c_str(), params_str.c_str(), handler_response,
      ...
      if (is_json_rpc_response_envelope(handler_json)) {
        resp = std::move(handler_json);
      } else if (is_valid_json(handler_json)) {
        resp = build_json_rpc_result(id_json, handler_json);
      } else {
        server_log(server, MCP_LOG_ERROR, "Handler returned invalid JSON for method=%s",
                   method.c_str());
        resp = build_json_rpc_error(id_json, -32603, "Internal error");
      }
  ...
  if (!handled) {
    ...
    // Method not found
    server_log(server, MCP_LOG_WARN, "Method not found: %s", method.c_str());
    std::string resp = build_json_rpc_error(id_json, -32601, "Method not found");
    if (resp.size() >= response_size) {
      *http_status = 500;
      return false;
    }
...
static bool handle_health_request(Server* server, char* response_buffer, size_t response_size,
                                  int* http_status) {
  if (!server || !response_buffer || !http_status) {
    return false;
  }

  server_log(server, MCP_LOG_DEBUG, "HTTP GET %s", MCP_ENDPOINT_HEALTH);
  Builder b;
  b.start_object();
  b.add("status", "ok");
...
static bool handle_route_mcp(void* user_data, const char* method, const char* path,
  ...
  if (std::strcmp(method, "DELETE") == 0) {
    ...
    (void)delete_session_by_id(server, session_id);
    response_buffer[0] = '\0';
    *http_status       = 204;
    server->requests_handled++;
    return true;
  ...
  return handle_mcp_request(server, body ? body : "", response_buffer, response_size, http_status);
}

static bool handle_route_health(void* user_data, const char* method, const char* path,
                                const char* body, char* response_buffer, size_t response_size,
                                int* http_status) {
  (void)path;
  (void)body;
  ...
  return handle_health_request(server, response_buffer, response_size, http_status);
}

static bool dispatch_http_route(Server* server, const char* method, const char* path,
                                const char* body, char* response_buffer, size_t response_size,
                                int* http_status) {
  ...
  RouteHandler handler{};
  {
    ...
    auto                        it = server->routes.find(build_route_key(method, path));
    if (it == server->routes.end()) {
      server_log(server, MCP_LOG_WARN, "Route not found: %s %s", method, path);
      return false;
    }
    handler = it->second;
  }
...
static bool on_http_request(void* user_data, const char* method, const char* path, const char* body,
  ...
  server_log(server, MCP_LOG_DEBUG, "HTTP %s %s", method, path);

  response_buffer[0] = '\0';
  *http_status       = 404;
  return dispatch_http_route(server, method, path, body, response_buffer, response_size,
                             http_status);
}

static void* on_sse_connect(void* user_data, void** client_handle) {
  (void)client_handle;
  auto* server = static_cast<Server*>(user_data);
  if (server) {
    uint64_t clients = server->connected_clients.fetch_add(1, std::memory_order_relaxed) + 1;
    server_log(server, MCP_LOG_DEBUG, "SSE client connected (clients=%llu)",
               static_cast<unsigned long long>(clients));
  }
  return nullptr;
}

static void on_sse_disconnect(void* user_data, void* client_handle) {
  (void)client_handle;
  auto* server = static_cast<Server*>(user_data);
  if (!server) return;
  ...
  while (current > 0) {
    if (server->connected_clients.compare_exchange_weak(
      ...
      server_log(server, MCP_LOG_DEBUG, "SSE client disconnected (clients=%llu)",
...
mcp_server_t* mcp_server_create(const mcp_server_config_t* config) {
  ...
  server->config = mcp_server_config_default();
  if (config) {
  ...
  {
    std::lock_guard<std::mutex> lock(server->routes_mutex);
    server->routes[mcp::build_route_key("POST", MCP_ENDPOINT_MCP)]   = {mcp::handle_route_mcp,
                                                                        server.get()};
    server->routes[mcp::build_route_key("DELETE", MCP_ENDPOINT_MCP)] = {mcp::handle_route_mcp,
                                                                        server.get()};
    server->routes[mcp::build_route_key("GET", MCP_ENDPOINT_HEALTH)] = {mcp::handle_route_health,
                                                                        server.get()};
  }

  if (!server->config.start_transport) {
    mcp::server_log(server.get(), MCP_LOG_INFO, "MCP server created without HTTP/SSE transport");
  ...
  callbacks.on_http_request           = mcp::on_http_request;
  callbacks.on_sse_connect            = mcp::on_sse_connect;
  callbacks.on_sse_disconnect         = mcp::on_sse_disconnect;
  ...
  if (!transport_api->create || !transport_api->destroy || !transport_api->start ||
    ...
    mcp::server_log(server.get(), MCP_LOG_ERROR, "Invalid MCP transport implementation");
    return nullptr;
  }
  ...
  if (!transport) {
    mcp::server_log(server.get(), MCP_LOG_ERROR, "Failed to create transport=%s port=%u",
  ...
  if (!transport_api->start(server->transport)) {
    ...
    mcp::server_log(server.get(), MCP_LOG_ERROR, "Failed to start transport=%s port=%u",
                    transport_api->name ? transport_api->name : "unknown", server->config.port);
    return nullptr;
  }
  ...
  server->running   = true;
  mcp::server_log(server.get(), MCP_LOG_INFO, "MCP server started on port=%u transport=%s",
...
void mcp_server_destroy(mcp_server_t* server_handle) {
  ...
  mcp::server_log(server.get(), MCP_LOG_INFO, "MCP server shutting down");
...
mcp_status_t mcp_server_route_register(mcp_server_t* server_handle, const char* method,
  ...
  std::string key    = mcp::build_route_key(method, path);

  if (mcp::is_reserved_route_key(key)) {
    return MCP_STATUS_ERROR(MCP_STATUS_CODE_INVALID_ARGS, "Reserved route");
  }

  std::lock_guard<std::mutex> lock(server->routes_mutex);
  server->routes[key] = {handler, user_data};
  mcp::server_log(server, MCP_LOG_DEBUG, "Registered route=%s %s", method, path);
...
void mcp_server_route_unregister(mcp_server_t* server_handle, const char* method,
  ...
  mcp::server_log(server, MCP_LOG_DEBUG, "Unregistered route=%s %s", method, path);
}
...
mcp_status_t mcp_server_method_register(mcp_server_t* server_handle, const char* method,
  ...
  mcp::server_log(server, MCP_LOG_DEBUG, "Registered method=%s", method);
  return MCP_STATUS_OK("Success");
}
...
mcp_status_t mcp_server_methods_register(mcp_server_t*                    server_handle,
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    mcp::server_log(server, MCP_LOG_DEBUG, "Registered method (batch)=%s", methods[i].method);
  }
...
void mcp_server_method_unregister(mcp_server_t* server_handle, const char* method) {
  ...
  mcp::server_log(server, MCP_LOG_DEBUG, "Unregistered method=%s", method);
}
...
void mcp_server_event_broadcast(mcp_server_t* server_handle, const char* event_type,
  ...
  if (mcp::is_json_rpc_message(json_payload)) {
    message_json = json_payload;
  } else {
    std::string method = std::string("notifications/") + event_type;
    message_json       = mcp::build_json_rpc_notification(method, json_payload);
  }
...
size_t mcp_format_success_response(char* buffer, size_t buffer_size, const char* id,
  ...
  if (!mcp::is_valid_json(result_part)) {
    result_part = "{}";
  }

  std::string response = mcp::build_json_rpc_result(id_json, result_part);
...
size_t mcp_format_error_response(char* buffer, size_t buffer_size, const char* id, int error_code,
  ...
  std::string response = mcp::build_json_rpc_error(id_json, error_code,
