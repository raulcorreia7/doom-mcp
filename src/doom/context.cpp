...
static bool handle_tools_call(void* user_data, const char* /*method*/,
  ...
  if (tool_name == "get_game_state") {
    const std::string resp = build_response("Use SSE stream for real-time game state");
    if (resp.size() >= response_size) return false;
    std::strcpy(response_buffer, resp.c_str());
    return true;
  }
...
dmcp_context_t* dmcp_create(const dmcp_config_t* config) {
  ...
  mcp_server_register_method(ctx->server, "tools/list", 
                             dmcp::handle_tools_list, ctx.get());
  mcp_server_register_method(ctx->server, "tools/call",
                             dmcp::handle_tools_call, ctx.get());
