...
static yyjson_mut_val* player_to_json(yyjson_mut_doc* doc, const dmcp_player_t& p) {
  yyjson_mut_val* obj = yyjson_mut_obj(doc);
  yyjson_mut_obj_add_sint(doc, obj, "hp", p.hp);
  yyjson_mut_obj_add_sint(doc, obj, "armor", p.armor);
...
static yyjson_mut_val* enemy_to_json(yyjson_mut_doc* doc, const dmcp_enemy_t& e) {
  ...
  yyjson_mut_obj_add_str(doc, obj, "state", e.hp > 0 ? DMCP_STATUS_ALIVE : DMCP_STATUS_DEAD);
