...
static dmcp_player_input_action_t parse_action_string(std::string_view action_str) {
  if (action_str == input::forward) return DMCP_INPUT_FORWARD;
  if (action_str == input::backward) return DMCP_INPUT_BACKWARD;
  if (action_str == input::strafe_left) return DMCP_INPUT_STRAFE_LEFT;
  if (action_str == input::strafe_right) return DMCP_INPUT_STRAFE_RIGHT;
  if (action_str == input::turn_left) return DMCP_INPUT_TURN_LEFT;
  if (action_str == input::turn_right) return DMCP_INPUT_TURN_RIGHT;
  if (action_str == input::aim) return DMCP_INPUT_AIM;
  if (action_str == input::attack) return DMCP_INPUT_ATTACK;
  if (action_str == input::use) return DMCP_INPUT_USE;
  if (action_str == input::weapon) return DMCP_INPUT_WEAPON;
...
bool parse_player_input_command(const json_value& params, dmcp_command_t* out) {
  ...
  if (!has_only_fields(params, {"action", "value"})) {
    return false;
  }

  std::string_view action_str;
  if (read_optional_string(params, {"action"}, "", &action_str)) {
  ...
  if (out->data.input.action == DMCP_INPUT_AIM) {
    double angle = 0.0;
    if (!read_required_number(params, {"value"}, &angle)) {
      return false;
    }
  ...
  if (out->data.input.action == DMCP_INPUT_WEAPON) {
    std::int64_t slot = 0;
    if (!read_required_int(params, {"value"}, &slot)) {
      return false;
    }
    if (slot < 1 || slot > 7) {
      return false;
