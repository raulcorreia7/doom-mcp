...
bool parse_give_item_command(const json_value& params, dmcp_command_t* out) {
  ...
  if (!has_only_fields(params, {"item_class", "amount"})) {
    return false;
  }

  std::string_view item_class;
  if (!read_required_string(params, {"item_class"}, &item_class) ||
  ...
  if (!read_optional_int(params, {"amount"}, 1, &amount) || amount < 1 || amount > 1000) {
    return false;
  }
